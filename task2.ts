...
const normalizeData = <T extends HasId>(
  unnormalizedData: T[]
): NormalizedData<T> => {
  const postsIds = unnormalizedData.map((post) => post.id);
  const postsById: Record<string, T> = {};
  unnormalizedData.forEach((post) => {
    let newId = post.id.replace(/[",']/g, "");
    postsById[newId] = post;
  });
